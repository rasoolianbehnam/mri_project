...
def get_torso(img, block_sizes):
    if len(block_sizes) == 0:
        raise ValueError(f"block_sizes is empty.")
    if len(img.shape) > 2:
        grey_img = img.mean(axis=2).astype('uint8')
    else:
        grey_img = img.astype('uint8')
    cnts, thresh_image = None, None
    for block_size in block_sizes:
        thresh_image = cv2.adaptiveThreshold(grey_img, 1, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY, block_size, 0)
        # thresh_image = cv2.adaptiveThreshold(grey_img, 1, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY,
        # block_size, 0)
        erd_img = cv2.erode(thresh_image, np.ones((5, 5)))
        _, cnts, _ = cv2.findContours(erd_img.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        cnts = sorted(filter(filter_contour(thresh_image), cnts), key=lambda x: -len(x))
        if len(cnts):
            break
    return cnts, thresh_image
...
def get_torso_outline(x):
    ...
    second_image = np.zeros_like(thrshldd_img)
    _, cnts, _ = cv2.findContours(img_to_drw.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    cnts = sorted(cnts, key=lambda t: -len(t))
    _ = cv2.drawContours(second_image, cnts, 0, 1, -11)
    kernel = np.ones((10, 10))
    out_and_masked = second_image * thrshldd_img
...
def resize_image_old1(x, width):
    w, h, *_ = x.shape
    new_shape = h * width // w, width
    return cv2.resize(x, new_shape)


def resize_images_old2(args, dsize):
    return tuple(cv2.resize(x, dsize) for x in args)


def resize_image(args, size):
    return cv2.resize(args, size)
...
def prepare_binary_mask_for_multi_label_prediction(x):
    ...
    print(np.unique(x))
    y = (x > 0).astype('uint8')
    _, contours, _ = cv2.findContours(y.copy(), cv2.RETR_CCOMP, cv2.CHAIN_APPROX_SIMPLE)
    imt = np.zeros_like(y)
    cv2.drawContours(imt, contours, -1, 255, 10)
    X = []
    Y = []
    C1 = {}
    C2 = {}
    ...
    return np.concatenate([X, X_ordered], axis=1), np.array(Y).reshape(-1, 1), C2, C1, np.array(II)
...
def get_contours(x):
    _, contours, _ = cv2.findContours(x.astype('uint8'), cv2.RETR_CCOMP, cv2.CHAIN_APPROX_SIMPLE)
    return contours


def accuracy(cls, prd):
    return accuracy_score(cls.reshape(-1), prd.reshape(-1))


def precision(cls, prd):
    return precision_score(cls.reshape(-1), prd.reshape(-1), average=None)


def recall(cls, prd):
    return recall_score(cls.reshape(-1), prd.reshape(-1), average=None)
